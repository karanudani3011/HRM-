...
const startMatch = '  const handleDownloadMOU = (acc) => {';
const endMatch = '    `;';
...
let foundEnd = -1;
while(true) {
  let idx = code.indexOf(endStr, curr);
  if (idx === -1) break;
  if (idx > startIdx + 10000) {
    foundEnd = idx + endStr.length;
    break;
  }
  curr = idx + endStr.length;
}

if (startIdx !== -1 && foundEnd !== -1) {
  const newContent = `  const handleDownloadMOU = (acc) => {
  ...
  const finalCode = code.substring(0, startIdx) + newContent + code.substring(foundEnd);
  fs.writeFileSync('src/pages/AdminDashboard.jsx', finalCode);
  console.log('Successfully fixed the MOU parsing issue.');
} else {
